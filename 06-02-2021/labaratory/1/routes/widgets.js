...
//передаём присланные в теле POST запроса информацию о новом виджете в хранилище
    db.add(req.body, (err, data)=>{
        if(err)
            return res.send('Error add widget!');res.redirect('/widgets/'); /*В случае успешного добавления виджета перенаправляем
...
//Роут организующий отдачу странички для редактирования виджета. Полный путь: /widgets/edit
route.get('/edit/:id', (req, res, next)=>{
    db.find(parseInt(req.params.id), (err, data)=>{
        ...
        res.render('edit', {
            ...
            route_url:"/widgets", //Основная часть пути URL используется в href
            data:data //Непосредственно данные об потенциально редактируемом виджете
        });
    });

});

/*Роут организующий получение данных для редактирования конкретного виджета.
Идентификатор виджета задаётся в пути. Полный путь: /edit/:id */
route.post('/edit/:id', (req, res, next)=>{
    /*просим хранилище отредактировать конкретный виджет с id, который прислан в последней части
    пути */
    db.edit(parseInt(req.params.id), req.body, (err, data)=>{
        if (err || !data)
            return res.send('Error edit widget!');
        res.redirect('/widgets/'); /*В случае успешного редактирования виджета перенаправляем
клиента на страницу со всеми виджетами, а именно на путь: /widgets/ */
    });
});
